...
class DocumentService:
    ...
    def chunk_document(self, converted_doc, chunking_strategy: str = "default"):
        ...
        chunks = self.chunking_service.chunk_document(
            converted_doc.document, strategy=chunking_strategy
        )

        print("Now processing chunks...")
        processed_chunks = self.chunking_service.process_chunks(
            chunks, chunking_strategy=chunking_strategy
        )
    ...
    def get_embeddings_for_chunks(self, processed_chunks):
        ...
        for i, chunk in enumerate(processed_chunks):
            if i % 10 == 0:  # Print progress every 10 chunks
                print(
                    f"Processing embedding {i + 1}/{total} ({(i + 1) / total * 100:.1f}%)"
                )
    ...
    def process_and_store_document(
        ...
        chunks = self.convert_and_chunk_document(
            doc_path, chunking_strategy, save_intermediate
        )
